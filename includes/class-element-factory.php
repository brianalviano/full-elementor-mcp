...
class Full_Full_Elementor_MCP_Element_Factory {
	 ...
	 *
	 * Routes atomic widget types (prefixed with `e-`) through the atomic
	 * factory so they get the required `styles`, `interactions`, and
	 * `version` top-level keys plus a default `classes` prop. Without this,
	 * atomic widgets created via the universal `add-widget` tool render
	 * as broken empty boxes in the Elementor 4.0 editor.
	 *
	 * @since 1.0.0
	 *
	 * @param string $widget_type The widget type name (e.g. 'heading', 'button', 'e-heading').
	...
	public function create_widget( string $widget_type, array $settings = array() ): array {
		if ( self::is_atomic_widget_type( $widget_type ) ) {
			return $this->create_atomic_widget( $widget_type, $settings );
		}
	...
	/**
	 * Returns true if a widget type name belongs to Elementor 4.0 atomic
	 * elements (prefix `e-`).
	 *
	 * @since 1.6.0
	 *
	...
	public static function is_atomic_widget_type( string $widget_type ): bool {
		return 0 === strpos( $widget_type, 'e-' );
	}

	/**
	 * Returns true for elTypes that represent atomic containers.
	 *
	 * @since 1.6.0
	 *
	 ...
	 * @return bool
	 */
	public static function is_atomic_container_type( string $el_type ): bool {
		return in_array( $el_type, array( 'e-flexbox', 'e-div-block' ), true );
	}
