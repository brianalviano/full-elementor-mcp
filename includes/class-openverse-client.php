...
class Full_Full_Elementor_MCP_Openverse_Client {
	...
	public function search_images( array $params ) {
		if ( empty( $params['q'] ) ) {
			return new \WP_Error(
				'missing_query',
				__( 'The search query parameter is required.', 'full-elementor-mcp' )
			);
		}
	...
	private function make_request( string $url ) {
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Openverse API request failed: %s', 'full-elementor-mcp' ),
		...
		if ( 429 === $status_code ) {
			return new \WP_Error(
				'rate_limited',
				__( 'Openverse API rate limit reached. Anonymous access allows 100 requests/day and 5 requests/hour. Please wait before making more requests.', 'full-elementor-mcp' )
			);
		}
		...
		if ( $status_code < 200 || $status_code >= 300 ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Openverse API returned HTTP %d.', 'full-elementor-mcp' ),
					$status_code
				)
			);
		}
		...
		if ( null === $data ) {
			return new \WP_Error(
				'json_parse_error',
				__( 'Failed to parse Openverse API response.', 'full-elementor-mcp' )
			);
		}
