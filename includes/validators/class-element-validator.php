...
class Full_Elementor_MCP_Element_Validator {
	...
	public function validate( array $element ) {
		if ( empty( $element['id'] ) ) {
			return new \WP_Error( 'missing_id', __( 'Element is missing an ID.', 'full-elementor-mcp' ) );
		}

		if ( empty( $element['elType'] ) ) {
			return new \WP_Error( 'missing_el_type', __( 'Element is missing elType.', 'full-elementor-mcp' ) );
		}
		...
		if ( ! in_array( $element['elType'], $valid_types, true ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Invalid element type: %s.', 'full-elementor-mcp' ),
					$element['elType']
				)
			);
		}
		...
		if ( 'widget' === $element['elType'] && empty( $element['widgetType'] ) ) {
			return new \WP_Error( 'missing_widget_type', __( 'Widget element is missing widgetType.', 'full-elementor-mcp' ) );
		}
