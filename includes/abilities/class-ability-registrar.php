 ...
 * @since 1.0.0
 */
class Full_Full_Elementor_MCP_Ability_Registrar {
	...
	public function register_all(): array {
		...
		$query = new Full_Full_Elementor_MCP_Query_Abilities( $this->data, $this->schema_generator );
		$query->register();
		$this->ability_names = array_merge( $this->ability_names, $query->get_ability_names() );

		// Phase 2: Page CRUD abilities (P1).
		$pages = new Full_Full_Elementor_MCP_Page_Abilities( $this->data, $this->factory );
		$pages->register();
		$this->ability_names = array_merge( $this->ability_names, $pages->get_ability_names() );

		// Phase 2: Layout/container abilities (P1).
		$layout = new Full_Full_Elementor_MCP_Layout_Abilities( $this->data, $this->factory );
		$layout->register();
		...
		$svg_icons = new Full_Full_Elementor_MCP_Svg_Icon_Abilities( $this->data, $this->factory );
		$svg_icons->register();
		$this->ability_names = array_merge( $this->ability_names, $svg_icons->get_ability_names() );

		// Custom code abilities (CSS, JS, code snippets).
		$custom_code = new Full_Full_Elementor_MCP_Custom_Code_Abilities( $this->data, $this->factory );
		...
		$atomic_widgets = new Full_Full_Elementor_MCP_Atomic_Widget_Abilities( $this->data, $this->factory );
		...
		$atomic_layout = new Full_Full_Elementor_MCP_Atomic_Layout_Abilities( $this->data, $this->factory );
