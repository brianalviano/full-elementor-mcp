...
class Full_Elementor_MCP_Composite_Abilities {
			...
			array(
				...
				'input_schema'        => array(
					...
					'properties' => array(
						...
						'page_settings' => array(
							'type'        => 'object',
							'description' => __( 'Page-level Elementor settings (background, padding, etc.).', 'full-elementor-mcp' ),
						),
						'structure'     => array(
							'type'        => 'array',
							'description' => __( 'Declarative element tree. Each item has type (container|widget), settings, and optionally children (for containers) or widget_type (for widgets).', 'full-elementor-mcp' ),
							'items'       => array(
								'type'       => 'object',
								'properties' => array(
									'type'        => array(
	...
	public function execute_build_page( $input ) {
		...
		if ( empty( $structure ) || ! is_array( $structure ) ) {
			return new \WP_Error( 'missing_structure', __( 'The structure parameter is required and must be an array.', 'full-elementor-mcp' ) );
		}
