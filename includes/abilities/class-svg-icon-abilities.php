 ...
 * @since 1.2.0
 */
class Full_Full_Elementor_MCP_Svg_Icon_Abilities {
			...
			array(
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'svg_url'     => array(
							'type'        => 'string',
							'description' => __( 'External URL to an SVG file to download and import. Use this OR svg_content, not both.', 'full-elementor-mcp' ),
						),
						'svg_content' => array(
							'type'        => 'string',
							'description' => __( 'Raw SVG markup string (e.g. "<svg viewBox=\'0 0 24 24\'><path d=\'M12 ...\'/></svg>"). Use this OR svg_url, not both.', 'full-elementor-mcp' ),
						),
						'title'       => array(
							'type'        => 'string',
							'description' => __( 'Title for the SVG in the Media Library. Falls back to filename or "Custom SVG Icon".', 'full-elementor-mcp' ),
						),
					),
				),
				'output_schema'       => array(
					...
					'properties' => array(
						...
						'icon_object'   => array(
							'type'        => 'object',
							'description' => __( 'Ready-to-use Elementor icon object. Pass this directly as the selected_icon setting.', 'full-elementor-mcp' ),
							'properties'  => array(
								'value'   => array(
									'type'       => 'object',
									'properties' => array(
	...
	public function execute_upload_svg_icon( $input ) {
		...
		if ( empty( $svg_url ) && empty( $svg_content ) ) {
			return new \WP_Error(
				'missing_input',
				__( 'Either svg_url or svg_content is required.', 'full-elementor-mcp' )
			);
		}
		...
		if ( ! empty( $svg_url ) && ! empty( $svg_content ) ) {
			return new \WP_Error(
				'conflicting_input',
				__( 'Provide either svg_url or svg_content, not both.', 'full-elementor-mcp' )
			);
		}
		...
		// Wrap the actual upload in try/finally so that an exception or fatal
		// from the WP media stack can never leave the SVG-allowing filters
		// hanging on the global filter chain (which would otherwise let any
		// later request upload arbitrary SVGs for the rest of the request).
		try {
			if ( ! empty( $svg_url ) ) {
				$result = $this->upload_from_url( $svg_url, $title );
			} else {
				$result = $this->upload_from_content( (string) $svg_content, $title );
			}
		} finally {
	...
	private function upload_from_url( string $url, string $title ): array {
		...
		if ( is_wp_error( $tmp_file ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to download SVG: %s', 'full-elementor-mcp' ),
		...
		// Sanitize the downloaded SVG before sideloading. Without this an
		// attacker-controlled URL could deliver SVG with embedded <script>,
		// `on*=` handlers, or `javascript:` hrefs that survive intact.
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
		$raw = file_get_contents( $tmp_file );
		if ( is_string( $raw ) && '' !== $raw ) {
			$sanitized = $this->sanitize_svg_content( $raw );
	...
	private function upload_from_content( string $content, string $title ): array {
		...
		if ( ! $tmp_file ) {
			return new \WP_Error(
				'temp_file_failed',
				__( 'Could not create temporary file for SVG upload.', 'full-elementor-mcp' )
			);
		}
	...
	private function do_sideload( string $tmp_file, string $filename ): array {
		...
		if ( is_wp_error( $attachment_id ) ) {
			...
			return new \WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'Failed to sideload SVG: %s', 'full-elementor-mcp' ),
	...
	private function validate_svg_file( string $file_path ) {
		...
		if ( empty( $content ) ) {
			return new \WP_Error(
				'empty_svg',
				__( 'The downloaded file is empty.', 'full-elementor-mcp' )
			);
		}
		...
		if ( stripos( $content, '<svg' ) === false ) {
			return new \WP_Error(
				'not_svg',
				__( 'The downloaded file does not contain SVG markup.', 'full-elementor-mcp' )
			);
		}
		...
		if ( preg_match( '/<script/i', $content ) ) {
			return new \WP_Error(
				'svg_has_script',
				__( 'SVG contains script elements and was rejected for security.', 'full-elementor-mcp' )
			);
		}
			...
			if ( $sanitizer && method_exists( $sanitizer, 'sanitize' ) ) {
				...
				if ( empty( $sanitized ) ) {
					return new \WP_Error(
						...
						__( 'SVG failed Elementor security sanitization.', 'full-elementor-mcp' )
					);
				}
	...
	private function sanitize_svg_content( string $content ) {
		...
		// Remove event handlers (on*="..."). Use `s` flag so `.` matches newlines
		// in multi-line attribute values.
		...
		if ( class_exists( '\Elementor\Utils' ) && method_exists( '\Elementor\Utils', 'get_svg_sanitizer' ) ) {
			...
			if ( $sanitizer && method_exists( $sanitizer, 'sanitize' ) ) {
				...
				if ( empty( $sanitized ) ) {
					return new \WP_Error(
						'svg_sanitization_failed',
						__( 'SVG failed Elementor security sanitization. Ensure it contains valid SVG markup.', 'full-elementor-mcp' )
					);
				}
				return $sanitized;
			}
