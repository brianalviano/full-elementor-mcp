...
class Full_Elementor_MCP_Query_Abilities {
	...
	public function get_ability_names(): array {
		return array(
			...
			'full-elementor-mcp/get-widget-schema',
			'full-elementor-mcp/get-container-schema',
			'full-elementor-mcp/get-page-structure',
			'full-elementor-mcp/get-element-settings',
			'full-elementor-mcp/find-element',
			'full-elementor-mcp/list-pages',
			'full-elementor-mcp/get-global-settings',
		);
	}
	...
	private function register_list_widgets(): void {
		full_elementor_mcp_register_ability(
			'full-elementor-mcp/list-widgets',
			array(
				'label'               => __( 'List Elementor Widgets', 'full-elementor-mcp' ),
				'description'         => __( 'Returns all registered Elementor widget types with their names, titles, icons, categories, and keywords. Optionally filter by widget category.', 'full-elementor-mcp' ),
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'category' => array(
							'type'        => 'string',
							'description' => __( 'Filter widgets by category slug.', 'full-elementor-mcp' ),
						),
					),
				),
				'output_schema'       => array(
	...
	private function register_get_widget_schema(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns the full JSON Schema for a widget type\'s settings, describing all available controls and their types. Use this to discover what settings a widget accepts before creating or updating it.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
	...
	public function execute_get_widget_schema( $input ) {
		...
		if ( empty( $widget_type ) ) {
			return new \WP_Error( 'missing_widget_type', __( 'The widget_type parameter is required.', 'full-elementor-mcp' ) );
		}
	...
	private function register_get_container_schema(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns JSON Schema for all container controls (flex + grid), including flex_direction, justify_content, align_items, flex_wrap, gap, content_width, min_height, container_type, grid controls, background, border, padding, and more.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
	...
	public function execute_get_container_schema( $input ) {
		...
		if ( ! $element_type ) {
			return new \WP_Error( 'container_not_found', __( 'Container element type not available.', 'full-elementor-mcp' ) );
		}
	...
	private function register_get_page_structure(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns the element tree for an Elementor page, showing all containers, widgets, and their nesting structure. Each element includes its ID, type, widget type (for widgets), and child elements.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'post_id' => array(
							'type'        => 'integer',
							'description' => __( 'The WordPress post/page ID.', 'full-elementor-mcp' ),
						),
					),
	...
	private function register_get_element_settings(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns the current settings for a specific element on a page. Provide the post ID and element ID to retrieve all control values for that element.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
				...
				'input_schema'        => array(
					...
					'properties' => array(
						...
						'element_id' => array(
							'type'        => 'string',
							'description' => __( 'The Elementor element ID.', 'full-elementor-mcp' ),
						),
					),
	...
	public function execute_get_element_settings( $input ) {
		...
		if ( empty( $element_id ) ) {
			return new \WP_Error( 'missing_element_id', __( 'The element_id parameter is required.', 'full-elementor-mcp' ) );
		}
		...
		if ( null === $element ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Element "%s" not found on this page.', 'full-elementor-mcp' ),
					$element_id
				)
			);
		}
	...
	private function register_find_element(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Searches elements on a page by type, widget type, or settings content. Returns matching element IDs, types, and a settings preview.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'post_id'       => array(
							'type'        => 'integer',
							'description' => __( 'The post/page ID to search.', 'full-elementor-mcp' ),
						),
						'widget_type'   => array(
							'type'        => 'string',
							'description' => __( 'Filter by widget type (e.g. "heading", "button"). Leave empty for all.', 'full-elementor-mcp' ),
						),
						'element_type'  => array(
							'type'        => 'string',
							'enum'        => array( 'container', 'widget' ),
							'description' => __( 'Filter by element type.', 'full-elementor-mcp' ),
						),
						'search_text'   => array(
							'type'        => 'string',
							'description' => __( 'Search for text content in settings values (case-insensitive).', 'full-elementor-mcp' ),
						),
						'setting_key'   => array(
							'type'        => 'string',
							'description' => __( 'Filter by setting key existence (e.g. "title_color").', 'full-elementor-mcp' ),
						),
						'setting_value' => array(
							'type'        => 'string',
							'description' => __( 'Filter by setting value (requires setting_key).', 'full-elementor-mcp' ),
						),
					),
					'required'   => array( 'post_id' ),
				),
	...
	private function register_list_pages(): void {
		full_elementor_mcp_register_ability(
			'full-elementor-mcp/list-pages',
			array(
				'label'               => __( 'List Elementor Pages', 'full-elementor-mcp' ),
				'description'         => __( 'Returns all WordPress pages and posts that are built with Elementor. Optionally filter by post type and status.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'post_type' => array(
							'type'        => 'string',
							'description' => __( 'Filter by post type (e.g. "page", "post"). Default: any.', 'full-elementor-mcp' ),
						),
						'status'    => array(
							'type'        => 'string',
							'description' => __( 'Filter by post status (e.g. "publish", "draft"). Default: any.', 'full-elementor-mcp' ),
						),
					),
				),
				'output_schema'       => array(
	...
	private function register_get_global_settings(): void {
		full_elementor_mcp_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns the active Elementor kit/global settings including colors, typography, spacing, and breakpoints. These are the site-wide design tokens used across all pages.', 'full-elementor-mcp' ),
				'category'            => 'full-elementor-mcp',
